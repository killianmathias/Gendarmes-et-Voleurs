...
size_t compute_next_position_cops (game * self, size_t index)
{
  if (self->robbers.size != 1)
    ...
    // Récupération des voisins du voleur (seul voleur)
    board_vertex *robber_vertex = self->robbers.positions[0];
    size_t robber_neighbors_count = robber_vertex->degree;

    for (size_t i = 0; i < self->b.vertices[index]->degree; i++) {
        size_t candidate = self->b.vertices[index]->neighbors[i]->index;

        // Distance au voleur actuel
        size_t dist_to_robber = board_dist(&self->b, candidate, robber_vertex->index);

        // Score : on cherche à réduire la distance au voleur
        // mais aussi à couvrir ses voisins (bloquer ses mouvements)
        // On compte combien de voisins du voleur sont proches du candidat
        size_t neighbors_covered = 0;
        for (size_t j = 0; j < robber_neighbors_count; j++) {
            size_t robber_neighbor_index = robber_vertex->neighbors[j]->index;
            // Si le candidat est proche du voisin du voleur (distance 1 ou 2 par ex.)
            size_t dist_to_robber_neighbor = board_dist(&self->b, candidate, robber_neighbor_index);
            if (dist_to_robber_neighbor <= 2) {
                neighbors_covered++;
            }
        }

        // Calcul du score : plus le candidat est proche du voleur et
        // plus il couvre ses voisins, plus c'est intéressant
        // On combine avec une pondération : distance faible est mieux,
        // nombre de voisins couverts élevé est mieux (donc on retire du score)
        size_t candidate_score = dist_to_robber * 10 - neighbors_covered * 5;
        ...
        // Choisir la meilleure option (score le plus faible)
        if (!already_used && candidate_score < score) {
