      ...
      for (size_t i = 0; i < self->b.vertices[index]->degree; i++)
        {
          ...
          size_t dist_to_robber =
            board_dist (&self->b, candidate, robber_vertex->index);
          ...
          for (size_t j = 0; j < robber_neighbors_count; j++)
            {
              size_t robber_neighbor_index =
                robber_vertex->neighbors[j]->index;
              // Si le candidat est proche du voisin du voleur (distance 1 ou 2 par ex.)
              size_t dist_to_robber_neighbor =
                board_dist (&self->b, candidate, robber_neighbor_index);
          ...
          size_t candidate_score =
            dist_to_robber * 10 - neighbors_covered * 5;
