...
void compute_zones(game *self){
  ...
  for (size_t i = 0; i<self->b.cops;i++){
    ...
    self->zone[i][1] = i * number_of_vertices_per_zone + number_of_vertices_per_zone-1;
  }
}
...
size_t compute_next_position_cops (game * self, size_t index)
{
  ...
  size_t next_position = board_next (&self->b, self->cops.positions[index]->index,
                     self->robbers.positions[self->targets[index]]->index);
  if (next_position >= self->zone[index][0] && next_position<= self->zone[index][1]){
